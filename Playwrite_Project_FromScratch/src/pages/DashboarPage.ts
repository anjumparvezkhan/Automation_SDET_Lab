...
export class DashboardPage {
  ...
  async isSidebarVisible(): Promise<boolean> {
    return await expect(this.dashboardSidebar).toBeVisible({ timeout: 5000 }).then(() => true).catch(() => false);
  }

  async isAdminMenuClickable(): Promise<boolean> {
    return await expect(this.adminMenu).toBeEnabled({ timeout: 5000 }).then(() => true).catch(() => false);
  }

  async isPimMenuClickable(): Promise<boolean> {
    return await expect(this.pimMenu).toBeEnabled({ timeout: 5000 }).then(() => true).catch(() => false);
  }

  async isMyInfoMenuClickable(): Promise<boolean> {
    return await expect(this.myInfoMenu).toBeEnabled({ timeout: 5000 }).then(() => true).catch(() => false);
  }
